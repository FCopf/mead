...
rg2 = rg1 +
    ...
    annotate(
    "text", x = 1250, y = 1250,
    label = as.expression(bquote(.(round(Ym,0)) ~ " = " ~ .(round(b0,0)) ~ " + " ~ .(round(b1,2)) ~ " x " ~ .(Xm))),
    size = 4
    ) +
    theme_classic()
...
```{r fig.align = "center", fig.width=5, fig.height=6, warning=F}
#| echo: false
...
rg1 = ggplot(DS, aes(y = Y, x = X)) +
  geom_point() +
  geom_smooth(method = lm, se = FALSE) +
  geom_segment(aes(y = Y, x = X, xend = X, yend = Yfit),
  ...
  annotate(
    ...
    y = 5,
    label = as.expression(
      bquote(sum(epsilon[i]^2) ~ "=" ~  sum((Y[i] - hat(Y[i]))^2) ~ "=" ~ .(floor(sre)))
    ),
    size = 3
  ) +
  theme_classic()
...
rg2 = ggplot(DS, aes(y = Y, x = X)) +
  geom_point() +
  geom_smooth(aes(y = Y2), method = lm, se = FALSE) +
  geom_segment(aes(y = Y, x = X, xend = X, yend = Yfit2),
               colour = "red", linetype = 3) +
  annotate(
    "text",
    x = 2.5,
    y = 5,
    label = as.expression(
      bquote(sum(epsilon[i]^2) ~ "=" ~  sum((Y[i] - hat(Y[i]))^2) ~ "=" ~ .(floor(sre2)))
    ),
    size = 3
  ) +
  theme_classic()
...
reg1 <- ggplot(df, mapping = aes(y = Y1, x = X)) +
  ...
  ylim(limitey) +
  annotate(
    "text", x = 30, y = 250, size = 4,
    label = as.expression(bquote(SQ["Regressão"] ~ ">>>>"))
  ) +
  theme_classic()
...
reg2 <- ggplot(df, mapping = aes(y = Y2, x = X)) +
  ...
  annotate(
    "text", x = 30, y = 250, size = 4,
    label = as.expression(bquote(SQ["Resíduo"] ~ ">>>>"))
  ) +
  theme_classic()
...
```{r, fig.align='center', fig.height=15, fig.width=6}
#| echo: false
...
Flim1 <- qf(p = 0.05, df1 = num1, df2 = den1, lower.tail = FALSE)
Flim2 <- qf(p = 0.05, df1 = num2, df2 = den2, lower.tail = FALSE)
...
lab1 <- as.expression(bquote(gl[numerador] == .(num1) * ";" ~ gl[denominador] == .(den1)))
lab2 <- as.expression(bquote(gl[numerador] == .(num2) * ";" ~ gl[denominador] == .(den2)))

gdfa <- ggplot(DF) +
  geom_line(aes(x = f, y = DF1), color = "blue") +
  geom_line(aes(x = f, y = DF2), color = "red") +
  ...
  annotate("text", x = 6.5, y = 1, size = 4, label = lab1) +
  annotate("text", x = 6.5, y = 0.85, size = 4, label = lab2) +
...
gdfb <- ggplot(DF) +
  ...
  ylab("Densidade de probabilidade de F") +
  xlab("Valores de F") +
  annotate("text", x = 6, y = 1, label = lab1) +
  geom_segment(aes(x = Flim1, xend = Flim1, y = 0.3, yend = 0.1),
...
gdfc <- ggplot(DF) +
  ...
  ylab("Densidade de probabilidade de F") +
  xlab("Valores de F") +
  annotate("text", x = 6, y = 0.85, label = lab2) +
  geom_segment(aes(x = Flim2, xend = Flim2, y = 0.3, yend = 0.2),
               colour = "red", arrow = arrow(length = unit(0.1, "inches"))) +
  theme_classic(base_size = 20)
...
Lembre-se que já estimamos anteriormente a variância residual destes dados ($s^2 = `r round(s2res,2)`$). Como temos `r n` observações, o valor de $t_{(\alpha, n-2)} = `r round(qt95,2)`$, portanto:

$s_{Y|X} = \sqrt{s^2(\frac{1}{n} + \frac{(X_i-\overline{X})^2}{SQ_X})} = `r round(epy[obs],2)`$

O valor estimado de riqueza neste ponto é `r round(rksR$Yfit[4],2)`, portanto:

$IC_{Y} = \hat{Y} \pm t_{(\alpha, n-2)} \times s_{Y|X} = `r round(Yfit[obs],2)` \pm `r round(qt95,2)` \times `r round(epy[obs],2)`$
